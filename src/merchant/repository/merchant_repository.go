...
type MerchantRepository interface {
	...
	FindOneBySlug(ctx context.Context, db *gorm.DB, slug string) (model.Merchant, error)
...
func (repository *merchantRepositoryImpl) FindOneBySlug(ctx context.Context, db *gorm.DB, slug string) (model.Merchant, error) {
	var merchant model.Merchant
	err := db.WithContext(ctx).First(&merchant, "slug = ?", slug).Error
	if err != nil {
		return model.Merchant{}, err
	}
	return merchant, nil
